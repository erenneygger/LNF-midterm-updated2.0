...
public class records extends javax.swing.JFrame {
  ...
  public records() {        
    ...
    // Updated to highlight the Settings/Records button
    setColor(Settings); 
    
    // Loads the claimed item data into your 5-column table
...
void displayUser() {
    ...
    // We add 'AS owner_name' so the table header stays clean
    // We use 'date_claimed' as the source for the date
    String sql = "SELECT item_id, (owner_fname || ' ' || owner_lname) AS owner_name, "
               + "owner_email, owner_contact, date_claimed " 
               + "FROM tbl_claims ORDER BY date_claimed DESC";
    
    try {
        cn.displayData(sql, usertable);
        System.out.println("DEBUG: Table refreshed successfully.");
    } catch (Exception e) {
    ...
    private void initComponents() {
        ...
        jLabel1.setText("RECORDS LOGS");
        jPanel2.add(jLabel1, new org.netbeans.lib.awtextra.AbsoluteConstraints(320, 10, -1, 104));

        jLabel2.setIcon(new javax.swing.ImageIcon(getClass().getResource("/Images/test1.png"))); // NOI18N
        jPanel2.add(jLabel2, new org.netbeans.lib.awtextra.AbsoluteConstraints(80, -10, 270, 160));
        ...
        usertable.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Item ID", "Owner Name", "Email", "Contact No", "Date Claimed"
            }
        ) {
    ...
    // Added the same 'AS owner_name' here to keep the table structure consistent
    String sql = "SELECT item_id, (owner_fname || ' ' || owner_lname) AS owner_name, "
               + "owner_email, owner_contact, date_claimed "
               + "FROM tbl_claims "
               + "WHERE owner_fname LIKE '%" + keyword + "%' "
               + "OR owner_lname LIKE '%" + keyword + "%' "
